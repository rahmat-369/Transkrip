...
function toastStyles(type) {
  if (type === "success") {
    return { ring: "ring-1 ring-emerald-200/70", bg: "bg-emerald-50/75", text: "text-emerald-950", sub: "text-emerald-900/70", iconWrap: "bg-emerald-100 text-emerald-700", bar: "bg-emerald-500" };
  }
  if (type === "error") {
    return { ring: "ring-1 ring-red-200/70", bg: "bg-red-50/75", text: "text-red-950", sub: "text-red-900/70", iconWrap: "bg-red-100 text-red-700", bar: "bg-red-500" };
  }
...
function CustomSelect({ value, onChange, options, disabled = false }) {
  ...
  return (
    <div ref={ref} className="relative z-[110]">
      ...
      <div
        className={["absolute left-0 right-0 mt-2 overflow-hidden rounded-2xl border border-gray-200 bg-[#FFFFFF] shadow-[0_18px_40px_-26px_rgba(0,0,0,0.35)]", "transition-all duration-200", open ? "opacity-100 translate-y-0 pointer-events-auto" : "opacity-0 -translate-y-1 pointer-events-none"].join(" ")}
        style={{ zIndex: 120 }}
      >
        <div className="p-1">
          {options.map((o) => {
            ...
            return (
              <button
                ...
                className={["w-full text-left px-3 py-2.5 rounded-xl text-sm font-bold transition", active ? "bg-gray-900 text-white" : "text-gray-800 hover:bg-gray-50"].join(" ")}
                style={{ WebkitTapHighlightColor: "transparent" }}
              >
                {o.label}
              </button>
...
function TypingDots() {
  return (
    <div className="flex items-center gap-1.5">
      <span className="h-1.5 w-1.5 rounded-full bg-indigo-500/70 animate-bounce" style={{ animationDelay: "0ms" }} />
      <span className="h-1.5 w-1.5 rounded-full bg-indigo-500/70 animate-bounce" style={{ animationDelay: "120ms" }} />
      <span className="h-1.5 w-1.5 rounded-full bg-indigo-500/70 animate-bounce" style={{ animationDelay: "240ms" }} />
    </div>
  );
}
  ...
  return (
    <div className={["fixed inset-0 z-[95] transition-all duration-300", open ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"].join(" ")}>
      ...
      <div className="absolute inset-0 flex items-end sm:items-center justify-center p-4">
        <div
          className={["w-full", widthClass, "rounded-3xl overflow-hidden", "bg-white border border-gray-200 shadow-2xl", "transform transition-all duration-300", open ? "translate-y-0 scale-100" : "translate-y-6 scale-[0.96]"].join(" ")}
        ...
        >
          <div className="px-5 py-4 border-b border-gray-200 bg-gradient-to-b from-white to-gray-50 flex items-center justify-between">
            ...
            <button
              ...
              style={{ WebkitTapHighlightColor: "transparent" }} aria-label="Tutup"
            >
              <X size={18} />
            </button>
          </div>
...
export default function App() {
  const imgDevProfile = "/image/rhmt-icon.png"; // Foto profile dev hanya muncul di info & kontak
  const clickSfx = "https://files.catbox.moe/tk3jff.mp3";

  const ADMIN = {
    waLink: "https://chat.whatsapp.com/IfWRoOW9H4HAHj689XKH4M?mode=hqctcla",
    telegramLink: "https://t.me/rAi_engine",
  ...
  const [chatMessages, setChatMessages] = useState([
    ...
    { role: "assistant", content: "Aku adalah AI asisten yang dikembangkan oleh R_hmt ofc. Tempel transkrip, pilih mode output, aku bikinin hasil siap edit." },
  ]);
  ...
  useEffect(() => { if (!chatOpen) return; setTimeout(() => scrollChatToBottom(false), 0); }, [chatOpen]);
  useEffect(() => { if (!chatOpen) return; scrollChatToBottom(true); }, [chatMessages.length, chatSending, chatOpen]);

  const dismissToast = (id) => {
    setToasts((prev) => prev.map((t) => (t.id === id ? { ...t, leaving: true } : t)));
    const timer = setTimeout(() => { setToasts((prev) => prev.filter((t) => t.id !== id)); toastTimers.current.delete(id); }, reduceMotion ? 0 : 650);
  ...
  useEffect(() => {
    return () => { for (const t of toastTimers.current.values()) clearTimeout(t); toastTimers.current.clear(); };
  }, []);

  const loadTranscriptHistory = () => {
    const items = safeJsonParse(localStorage.getItem(HISTORY_KEYS.transcript) || "[]", []);
    setTranscriptHistory(Array.isArray(items) ? items.slice(0, 50) : []);
  };

  const loadChatHistory = () => {
    const items = safeJsonParse(localStorage.getItem(HISTORY_KEYS.chat) || "[]", []);
    setChatHistory(Array.isArray(items) ? items.slice(0, 50) : []);
  };
  ...
  const fetchTranscript = async (e) => {
    ...
    if (!v.ok) { pushToast({ type: "error", title: "Gagal validasi", message: v.reason }); return; }
    ...
    try {
      ...
      if (data?.status && data?.data?.transcript) {
        ...
        saveTranscriptHistoryItem({ id: `${Date.now()}_${Math.random().toString(16).slice(2)}`, ts: Date.now(), url: url.trim(), transcript: t, preview: t.slice(0, 220) });
      } else {
        pushToast({ type: "error", title: "Gagal", message: data?.message || "Transkrip tidak tersedia untuk video ini." });
      }
    } catch { pushToast({ type: "error", title: "Jaringan bermasalah", message: "API tidak dapat diakses." }); } finally { setLoading(false); }
  };
  ...
  const resetAll = () => { setTranscript(""); setUrl(""); setCopied(false); pushToast({ type: "info", title: "Reset", message: "Input dan hasil dibersihkan.", duration: 2000 }); };

  const getModePrompt = (mode, textHasTs) => {
    if (mode === "clip") {
      return textHasTs ? `Tugas: bantu clipper bikin potongan konten dari transkrip. Output WAJIB: 1) 10 kandidat clip (format: [Clip #] Timestamp start-end, Durasi, Judul, Hook, Ringkas, Cut instruction). 2) 5 judul Shorts. 3) 10 caption + hashtag. Bahasa Indonesia kasual.` : `Tugas: bantu clipper bikin potongan konten dari transkrip. PENTING: transkrip ini tidak punya timestamp. Jangan bikin timestamp. Output WAJIB: 1) 10 kandidat clip (format: [Clip #] Judul, Durasi estimasi, Hook, Ringkas, Cut instruction). 2) 5 judul Shorts. 3) 10 caption + hashtag. Bahasa Indonesia kasual.`;
    }
    if (mode === "chapters") {
      return textHasTs ? `Tugas: bikin chapters + title pack. Output: - Chapters format: mm:ss Judul - 3 opsi judul video panjang - 3 opsi thumbnail text (2-4 kata)` : `Tugas: bikin struktur chapter tanpa waktu. PENTING: Jangan bikin waktu/perkiraan. Output: - Chapters format: (1) Judul, (2) Judul, dst - 3 opsi judul video panjang - 3 opsi thumbnail text (2-4 kata)`;
    }
    if (mode === "summary") { return `Tugas: ringkas transkrip jadi struktur. Output: - TL;DR (3 kalimat) - Outline (7-12 poin) - 8 quote yang paling kuat (exact words) - CTA yang cocok.`; }
    if (mode === "rewrite") { return `Tugas: ubah transkrip jadi naskah voiceover yang rapih tanpa ubah makna. Output: - Naskah paragraf pendek - 5 opsi hook (1 kalimat)`; }
    return `Tugas: bantu clipper dari transkrip.`;
  };

  const buildClipperPrompt = (text, userCustom, mode) => {
    const textHasTs = hasTimestamps(text);
    const basePrompt = `Kamu adalah asisten AI untuk creator Indonesia. Aturan: - Jangan ngajarin teori. - Jangan panjang-panjang. - Kalau transkrip tidak punya timestamp: jangan bikin timestamp. - Jangan pakai emoji.`;
  ...
  const sendChat = async () => {
    ...
    if (!text) { pushToast({ type: "error", title: "Kosong", message: "Tempel transkrip di kolom chat." }); return; }
    ...
    } catch { setChatMessages((prev) => [...prev, { role: "assistant", content: "Error: API tidak bisa diakses." }]); } finally { setChatSending(false); }
  };
  ...
  const copyAssistantMessage = async (idx) => {
    ...
    try { await navigator.clipboard.writeText(msg.content); setCopiedMsgId(idx); setTimeout(() => setCopiedMsgId(null), 1200); }
    catch { pushToast({ type: "error", title: "Gagal", message: "Clipboard ditolak." }); }
  };
  ...
  const restoreChatHistoryItem = (item) => {
    ...
    setChatMessages([user, assistant]); setChatMode(item.mode || "clip"); setChatCustomPrompt(item.custom || ""); setHistoryOpen(false); setChatOpen(true); setFabOpen(false);
    setTimeout(() => scrollChatToBottom(false), 50);
  };

  const restoreTranscriptHistoryItem = (item) => {
    setUrl(item.url || ""); setTranscript(item.transcript || ""); setCopied(false); setHistoryOpen(false);
    pushToast({ type: "success", title: "Dipulihkan", message: "Transkrip dimuat dari history.", duration: 1600 });
  };

  const copyText = async (t) => {
    try { await navigator.clipboard.writeText(t); pushToast({ type: "success", title: "Tersalin", message: "Disalin ke clipboard.", duration: 1600 }); }
    catch { pushToast({ type: "error", title: "Gagal", message: "Clipboard ditolak." }); }
  };
  ...
  return (
    <div
      className={`min-h-[100dvh] flex flex-col text-[#111111] font-sans selection:bg-indigo-500/30 selection:text-indigo-900 relative transition-opacity duration-1000 ${
    ...
    >
      ...
      {/* Optimized Background (No heavy blur, zero lag) */}
      <div className="fixed inset-0 z-0 pointer-events-none">
        <div className="absolute inset-0 bg-[#FBFBFB]" />
        {/* Soft Grid */}
        <div
          className="absolute inset-0 opacity-[0.25]"
          style={{ backgroundImage: "linear-gradient(to right, rgba(0,0,0,0.06) 1px, transparent 1px), linear-gradient(to bottom, rgba(0,0,0,0.06) 1px, transparent 1px)", backgroundSize: "40px 40px" }}
        />
        {/* Static Soft Light (replaces heavy blur) */}
      ...
      <div className="fixed top-4 right-4 z-[90] w-[min(360px,calc(100vw-2rem))] space-y-2">
        {toasts.map((t) => {
          ...
          return (
            <div
              key={t.id}
              className={["relative overflow-hidden rounded-2xl", "bg-white/95 backdrop-blur-md shadow-[0_18px_44px_-26px_rgba(0,0,0,0.55)]", "will-change-transform", s.ring].join(" ")}
              style={{ animation: reduceMotion ? "none" : t.leaving ? "toastOutSoft 650ms cubic-bezier(.22,1,.36,1) forwards" : "toastInSoft 900ms cubic-bezier(.22,1,.36,1) forwards" }}
            >
              <button onClick={() => dismissToast(t.id)} className="absolute top-2 right-2 h-8 w-8 grid place-items-center rounded-xl text-gray-500 hover:text-gray-900 hover:bg-black/5 active:scale-95 transition" aria-label="Tutup"><span className="text-lg leading-none">×</span></button>
              <div className="flex gap-3 p-4 pr-10">
                <div className={["h-9 w-9 rounded-xl grid place-items-center shrink-0", s.iconWrap].join(" ")}><ToastIcon type={t.type} /></div>
                <div className="min-w-0">
      ...
      {/* Header Sticky (Natural scrolling) */}
      <header className="sticky top-0 z-30 bg-[#FBFBFB]/85 backdrop-blur-xl border-b border-gray-200/60 shadow-sm transition-all duration-300">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3 group cursor-pointer" onClick={resetAll}>
            {/* Logo YouTube */}
            <div className="bg-[#111111] text-white p-2 rounded-xl group-hover:scale-105 group-hover:-rotate-2 transition-transform duration-300 shadow-md">
              <Youtube size={20} strokeWidth={2.5} className="group-hover:text-red-500 transition-colors duration-300" />
            </div>
            <h1 className="font-extrabold text-lg sm:text-xl tracking-tight text-gray-900 drop-shadow-sm">YouTube Transcript</h1>
          </div>

          <div className="flex items-center gap-2 text-xs font-semibold text-gray-600 bg-white/60 px-3 py-1.5 rounded-full border border-gray-200/70 cursor-default hover:bg-white transition-colors shadow-sm">
            <Zap size={14} className="text-indigo-500 fill-indigo-500 animate-pulse" />
      ...
      {/* Main Content (Flex-grow takes up available space, pushing footer down) */}
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          <section className={`w-full text-center space-y-6 sm:space-y-8 transition-all duration-700 ${isMounted ? "translate-y-0 opacity-100" : "translate-y-3 opacity-0"}`}>
            <div className="space-y-4">
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white border border-gray-200 shadow-sm text-sm font-bold text-gray-700 mb-2 hover:shadow-md transition-shadow cursor-default" style={{ animation: "bounce 3s infinite" }}>
                <Sparkles size={16} className="text-indigo-600" />
                Ekstrak otomatis dengan AI
              </div>

              <h2 className="text-4xl sm:text-5xl md:text-6xl font-black tracking-tight text-[#111111] leading-tight drop-shadow-sm px-2">
                ...
                <span className="relative inline-block mt-1 sm:mt-0">
                  ...
                  <svg className="absolute -bottom-2 sm:-bottom-4 left-0 w-full h-3 sm:h-5 text-neutral-900/20" viewBox="0 0 200 20" fill="none" preserveAspectRatio="none" aria-hidden="true">
            ...
            <form
              ...
              className="max-w-2xl mx-auto relative flex flex-col sm:flex-row gap-3 p-2 bg-white/95 rounded-2xl sm:rounded-full shadow-lg border border-gray-200/70 transition-all duration-300 hover:shadow-xl focus-within:ring-4 focus-within:ring-indigo-500/10 focus-within:border-indigo-300"
              style={{ WebkitTapHighlightColor: "transparent" }}
            >
              <div className="relative flex-grow flex items-center">
                ...
                <input
                  type="text" value={url} onChange={(e) => setUrl(e.target.value)} placeholder="tempel link youtube di sini..."
                  className="block w-full pl-14 pr-4 py-4 sm:py-0 bg-transparent text-sm sm:text-base font-semibold placeholder-gray-400 outline-none focus:outline-none focus:ring-0 transition-all"
                  style={{ WebkitTapHighlightColor: "transparent", WebkitAppearance: "none" }} disabled={loading} inputMode="url" autoCorrect="off" autoCapitalize="off" spellCheck={false}
                />
              </div>

              <button
                type="submit" disabled={loading}
                className="group relative flex items-center justify-center gap-2 py-4 px-8 font-bold rounded-xl sm:rounded-full text-white bg-gray-900 hover:bg-black active:scale-[0.98] transition-all duration-300 overflow-hidden disabled:opacity-70 disabled:cursor-not-allowed shadow-md"
              >
                ...
                {loading ? (
                  <><Loader2 className="h-5 w-5 animate-spin relative z-10" /><span className="relative z-10">Memproses...</span></>
                ) : (
                  <><span className="relative z-10">Ekstrak</span><Sparkles className="h-4 w-4 relative z-10 group-hover:rotate-12 transition-transform" /></>
                )}
              </button>
            </form>

            {/* Simple Guide for Laymen */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 max-w-3xl mx-auto mt-6 text-left px-4">
              <div className="p-4 bg-white/70 backdrop-blur-sm border border-gray-200 rounded-2xl flex items-start gap-3 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div>
                  <h4 className="font-extrabold text-sm text-gray-900">1. Tempel Link</h4>
                  <p className="text-xs font-semibold text-gray-600 mt-0.5 leading-relaxed">Salin tautan video dari YouTube dan tempel ke kolom di atas.</p>
                </div>
              </div>
              <div className="p-4 bg-white/70 backdrop-blur-sm border border-gray-200 rounded-2xl flex items-start gap-3 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div>
                  <h4 className="font-extrabold text-sm text-gray-900">2. Ekstrak Teks</h4>
                  <p className="text-xs font-semibold text-gray-600 mt-0.5 leading-relaxed">Sistem akan mengambil seluruh teks percakapan dari video.</p>
                </div>
              </div>
              <div className="p-4 bg-white/70 backdrop-blur-sm border border-gray-200 rounded-2xl flex items-start gap-3 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div>
                  <h4 className="font-extrabold text-sm text-gray-900">3. Chat AI</h4>
                  <p className="text-xs font-semibold text-gray-600 mt-0.5 leading-relaxed">Buka menu pojok kanan bawah, gunakan AI untuk meringkas.</p>
                </div>
              </div>
            </div>

            {/* Feature Badges (Mengisi layar kosong) */}
          ...
          <div className={`w-full max-w-3xl mx-auto mt-10 transition-all duration-700 ease-out transform origin-top ${transcript ? "opacity-100 translate-y-0 scale-100 block" : "opacity-0 translate-y-6 scale-95 pointer-events-none hidden"}`}>
            <section className="bg-white border border-gray-200 rounded-3xl shadow-xl shadow-black/5 overflow-hidden">
              <div className="flex items-center justify-between px-5 sm:px-8 py-4 border-b border-gray-200 bg-gray-50/80">
                <h3 className="font-extrabold text-gray-900 flex items-center gap-2.5">
                  <div className="p-1.5 bg-white rounded-lg shadow-sm text-gray-700 border border-gray-200"><FileText size={16} /></div>
                  Hasil Transkrip
                </h3>

                <button
                  onClick={handleCopyTranscript}
                  className={`flex items-center gap-2 px-4 py-2 text-sm font-bold rounded-xl border transition-all duration-200 active:scale-95 ${copied ? "bg-gray-900 border-gray-900 text-white shadow-sm" : "bg-white border-gray-200 text-gray-700 hover:bg-gray-50 shadow-sm"}`}
                  disabled={!transcript}
                >
                  {copied ? (<><Check className="h-4 w-4" /><span>Tersalin!</span></>) : (<><Copy className="h-4 w-4" /><span className="hidden sm:inline">Salin Teks</span><span className="sm:hidden">Salin</span></>)}
                </button>
              </div>
      ...
      {/* Footer Tenggelam (Sinking Footer) */}
      <footer className="mt-auto z-20 border-t border-gray-200 bg-[#FBFBFB]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex flex-col items-center sm:items-start gap-1">
            <div className="flex items-center gap-2 text-sm text-gray-900 font-extrabold">
          ...
          <div className="flex items-center gap-6 text-sm text-gray-600 font-bold">
            <a href={ADMIN.githubLink} className="hover:text-gray-900 transition-colors flex items-center gap-1.5 group">
              <Github size={16} className="group-hover:scale-110 transition-transform" />
              <span className="hidden sm:inline">GitHub</span>
            </a>
            <a href={ADMIN.tiktokLink} className="hover:text-gray-900 transition-colors flex items-center gap-1.5 group">
      ...
      <div className="fixed bottom-6 right-4 z-[80] flex flex-col items-end gap-3">
        ...
        <div className={`flex flex-col gap-3 transition-all duration-300 origin-bottom right-0 ${fabOpen ? "opacity-100 scale-100 pointer-events-auto" : "opacity-0 scale-90 pointer-events-none"}`}>
          <div className="flex items-center gap-3 justify-end">
            ...
            <button onClick={() => { setContactOpen(true); setFabOpen(false); }} className="h-12 w-12 rounded-full bg-white text-gray-700 border border-gray-200 shadow-lg grid place-items-center hover:bg-gray-50 active:scale-95 transition" aria-label="Kontak"><Headset size={20} /></button>
          </div>
          <div className="flex items-center gap-3 justify-end">
            ...
            <button onClick={openHistory} className="h-12 w-12 rounded-full bg-white text-gray-700 border border-gray-200 shadow-lg grid place-items-center hover:bg-gray-50 active:scale-95 transition" aria-label="History"><History size={20} /></button>
          </div>
          <div className="flex items-center gap-3 justify-end">
            ...
            <button onClick={() => { setAboutOpen(true); setFabOpen(false); }} className="h-12 w-12 rounded-full bg-white text-gray-700 border border-gray-200 shadow-lg grid place-items-center hover:bg-gray-50 active:scale-95 transition" aria-label="About"><InfoIcon size={20} /></button>
          </div>
          <div className="flex items-center gap-3 justify-end">
            <span className="bg-gray-900 px-3 py-1.5 rounded-xl shadow-md text-xs font-bold text-white">Chat Asisten AI</span>
            <button onClick={() => { setChatOpen(!chatOpen); setFabOpen(false); }} className="h-12 w-12 rounded-full bg-indigo-600 text-white shadow-lg shadow-indigo-500/30 grid place-items-center hover:bg-indigo-700 active:scale-95 transition" aria-label="Chat AI"><MessageCircle size={20} /></button>
          </div>
        </div>

        {/* Main FAB Toggle */}
        <button
          onClick={() => { if(chatOpen) setChatOpen(false); setFabOpen(!fabOpen); }}
          className="h-14 w-14 rounded-full bg-gray-900 text-white shadow-xl shadow-black/20 grid place-items-center hover:bg-black active:scale-95 transition-transform z-50 relative"
          style={{ WebkitTapHighlightColor: "transparent" }}
        >
      ...
      <DialogShell open={aboutOpen} onClose={() => setAboutOpen(false)} title="Tentang AI" subtitle="Asisten Konten Cerdas" icon={<Wand2 size={18} />}>
        <div className="flex items-center gap-4">
          <div className="h-14 w-14 rounded-full overflow-hidden border border-gray-200 bg-gray-50 shrink-0 shadow-inner">
            <img src={imgDevProfile} alt="Developer" className="w-full h-full object-cover" draggable={false} />
          </div>
          <div className="min-w-0">
            <div className="text-base font-extrabold text-gray-900">Developer Profile</div>
            <div className="text-xs font-bold text-gray-500 mt-0.5">R_hmt ofc</div>
            <div className="text-xs font-semibold text-gray-600 mt-1 leading-snug">Tool AI untuk mempermudah creator merangkum dan mencari ide konten.</div>
          </div>
        </div>
        ...
        <div className="mt-5 flex items-center justify-between">
          <div className="text-xs font-bold text-gray-400">Code Based by Zakrenz</div>
          <button onClick={() => setAboutOpen(false)} className="px-5 py-2.5 rounded-2xl bg-gray-900 text-white text-sm font-extrabold hover:bg-black active:scale-95 transition">Tutup</button>
        </div>
      </DialogShell>
      ...
      <DialogShell open={historyOpen} onClose={() => setHistoryOpen(false)} title="Riwayat" subtitle="Transkrip & Obrolan AI" icon={<History size={18} />} widthClass="max-w-[680px]">
        <div className="grid grid-cols-1 gap-4">
          ...
          {historyType === "transcript" ? (
            <>
              <div className="flex items-center justify-between gap-2">
                <div className="text-sm font-bold text-gray-700">Total Tersimpan: <span className="text-gray-900">{transcriptHistory.length}</span></div>
                <button onClick={clearTranscriptHistory} className="px-3 py-2 rounded-xl bg-red-50 text-red-600 text-xs font-extrabold hover:bg-red-100 active:scale-95 transition">Hapus Semua</button>
              </div>
              <div className="rounded-2xl border border-gray-200 overflow-hidden bg-gray-50">
                <div className="max-h-[350px] overflow-y-auto custom-scrollbar">
                  {transcriptHistory.length === 0 ? (
                    <div className="p-6 text-center text-sm font-semibold text-gray-500">Belum ada riwayat transkrip.</div>
                  ) : (
                    <div className="divide-y divide-gray-200">
                      {transcriptHistory.map((it) => (
                        <div key={it.id} className="p-4 bg-white hover:bg-gray-50 transition">
                          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                            <div className="min-w-0 flex-1">
                              <div className="text-[10px] font-bold text-gray-400">{new Date(it.ts).toLocaleString("id-ID")}</div>
                              <div className="mt-1 text-sm font-extrabold text-gray-900 truncate">{it.url || "-"}</div>
                              <div className="mt-1.5 text-xs font-semibold text-gray-600 whitespace-pre-wrap line-clamp-2">{it.preview || ""}</div>
                            </div>
                            <div className="flex items-center gap-2 shrink-0 mt-2 sm:mt-0">
                              <button onClick={() => restoreTranscriptHistoryItem(it)} className="px-4 py-2 rounded-xl bg-gray-900 text-white text-xs font-extrabold hover:bg-black active:scale-95 transition">Buka</button>
                              <button onClick={() => copyText(it.transcript || "")} className="h-8 w-8 rounded-xl border border-gray-200 bg-white grid place-items-center text-gray-700 hover:bg-gray-100 active:scale-95 transition"><Copy size={14} /></button>
                            </div>
                          </div>
                        </div>
                      ))}
            ...
            <>
              <div className="flex items-center justify-between gap-2">
                <div className="text-sm font-bold text-gray-700">Total Tersimpan: <span className="text-gray-900">{chatHistory.length}</span></div>
                <button onClick={clearChatHistory} className="px-3 py-2 rounded-xl bg-red-50 text-red-600 text-xs font-extrabold hover:bg-red-100 active:scale-95 transition">Hapus Semua</button>
              </div>
              <div className="rounded-2xl border border-gray-200 overflow-hidden bg-gray-50">
                <div className="max-h-[350px] overflow-y-auto custom-scrollbar">
                  {chatHistory.length === 0 ? (
                    <div className="p-6 text-center text-sm font-semibold text-gray-500">Belum ada riwayat chat.</div>
                  ) : (
                    <div className="divide-y divide-gray-200">
                      {chatHistory.map((it) => (
                        <div key={it.id} className="p-4 bg-white hover:bg-gray-50 transition">
                          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                            <div className="min-w-0 flex-1">
                              <div className="text-[10px] font-bold text-gray-400">{new Date(it.ts).toLocaleString("id-ID")} • <span className="text-indigo-600 uppercase tracking-wider">{String(it.mode || "clip")}</span></div>
                              <div className="mt-1.5 text-sm font-extrabold text-gray-900 line-clamp-2">{it.outputPreview || "(tanpa output)"}</div>
                            </div>
                            <div className="flex items-center gap-2 shrink-0 mt-2 sm:mt-0">
                              <button onClick={() => restoreChatHistoryItem(it)} className="px-4 py-2 rounded-xl bg-gray-900 text-white text-xs font-extrabold hover:bg-black active:scale-95 transition">Buka</button>
                              <button onClick={() => copyText(it.output || "")} className="h-8 w-8 rounded-xl border border-gray-200 bg-white grid place-items-center text-gray-700 hover:bg-gray-100 active:scale-95 transition"><Copy size={14} /></button>
                            </div>
                          </div>
                        </div>
                      ))}
          ...
          <div className="flex items-center justify-end mt-2">
            <button onClick={() => setHistoryOpen(false)} className="px-5 py-2.5 rounded-2xl bg-gray-200 text-gray-900 text-sm font-extrabold hover:bg-gray-300 active:scale-95 transition">Tutup</button>
          </div>
        </div>
      </DialogShell>
      ...
      <DialogShell open={contactOpen} onClose={() => setContactOpen(false)} title="Hubungi Admin" subtitle="Pilih platform di bawah" icon={<Headset size={18} />} widthClass="max-w-[480px]">
        {/* Developer Profile Header inside Contact */}
        <div className="flex items-center gap-4 mb-4 pb-4 border-b border-gray-100">
           <div className="h-12 w-12 rounded-full overflow-hidden border border-gray-200 bg-gray-50 shrink-0 shadow-inner">
            <img src={imgDevProfile} alt="Developer" className="w-full h-full object-cover" draggable={false} />
          </div>
          <div>
            <div className="text-sm font-extrabold text-gray-900">R_hmt ofc</div>
            <div className="text-[11px] font-bold text-gray-500">Developer & Creator</div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-gray-200 bg-gray-50 p-2 space-y-2">
          <a href={ADMIN.telegramLink} target="_blank" rel="noreferrer" className="w-full px-4 py-3.5 rounded-xl bg-white border border-gray-200 flex items-center justify-between gap-3 hover:bg-[#F0F8FF] hover:border-[#B0E0E6] active:scale-[0.98] transition-all group">
            <div className="min-w-0 flex items-center gap-3">
              <div className="h-10 w-10 rounded-full bg-[#0088cc] text-white flex items-center justify-center shrink-0"><TelegramIcon size={20} /></div>
              <div className="text-left"><div className="text-sm font-extrabold text-gray-900 group-hover:text-[#0088cc] transition-colors">Telegram</div><div className="text-xs font-bold text-gray-500 truncate">Tanya seputar AI & Tool</div></div>
            </div>
            <ExternalLink size={18} className="text-gray-400 group-hover:text-[#0088cc]/60 transition-colors" />
          </a>
          ...
          <a href={ADMIN.waLink} target="_blank" rel="noreferrer" className="w-full px-4 py-3.5 rounded-xl bg-white border border-gray-200 flex items-center justify-between gap-3 hover:bg-[#F0FFF4] hover:border-[#C6F6D5] active:scale-[0.98] transition-all group">
            <div className="min-w-0 flex items-center gap-3">
              <div className="h-10 w-10 rounded-full bg-[#25D366] text-white flex items-center justify-center shrink-0"><MessageCircle size={20} /></div>
              <div className="text-left"><div className="text-sm font-extrabold text-gray-900 group-hover:text-[#128C7E] transition-colors">WhatsApp</div><div className="text-xs font-bold text-gray-500 truncate">Join Chanel</div></div>
            </div>
            <ExternalLink size={18} className="text-gray-400 group-hover:text-[#25D366]/60 transition-colors" />
          </a>
          ...
          <a href={ADMIN.tiktokLink} target="_blank" rel="noreferrer" className="w-full px-4 py-3.5 rounded-xl border border-gray-200 bg-white flex items-center justify-between gap-3 hover:bg-gray-100 active:scale-[0.98] transition-all group">
            <div className="min-w-0 flex items-center gap-3">
              <div className="h-10 w-10 rounded-full bg-black text-white flex items-center justify-center shrink-0"><TikTokIcon size={18} /></div>
              <div className="text-left"><div className="text-sm font-extrabold text-gray-900 group-hover:text-black transition-colors">TikTok</div><div className="text-xs font-bold text-gray-500 truncate">Follow & Support</div></div>
            </div>
        ...
        <div className="mt-5 flex items-center justify-between">
          <div className="text-[11px] font-bold text-gray-400">Code Based by Zakrenz</div>
          <button onClick={() => setContactOpen(false)} className="px-5 py-2.5 rounded-2xl bg-gray-900 text-white text-sm font-extrabold hover:bg-black active:scale-95 transition">Tutup</button>
        </div>
      </DialogShell>
      ...
      <div
        className={[
          "fixed bottom-24 right-4 z-[75]",
          "w-[min(390px,calc(100vw-2rem))]",
          "flex flex-col",
          "transition-all duration-400 ease-out origin-bottom-right",
          chatOpen ? "opacity-100 scale-100 pointer-events-auto" : "opacity-0 scale-90 pointer-events-none translate-y-4",
        ].join(" ")}
        style={{ maxHeight: "calc(100dvh - 120px)" }} 
      >
        <div className="flex flex-col h-full rounded-3xl overflow-hidden border border-gray-200 bg-[#FFFFFF] shadow-[0_24px_60px_-15px_rgba(0,0,0,0.3)]">
          ...
          <div className="px-4 py-3 border-b border-gray-200 bg-[#FFFFFF] flex items-center justify-between shrink-0">
            <div className="flex items-center gap-2.5">
              ...
              <div className="min-w-0">
                <div className="text-sm font-extrabold text-gray-900 leading-tight">Asisten AI</div>
                <div className="text-[11px] font-bold text-indigo-600 leading-tight flex items-center gap-1">
          ...
          <div className="flex-1 overflow-y-auto bg-gray-50/50 p-4 space-y-3 custom-scrollbar" ref={chatListRef}>
                    ...
                    {isAssistant && (
                      <div className="pt-2 mt-2 border-t border-gray-100 flex items-center justify-end">
                        <button onClick={() => copyAssistantMessage(idx)} className="flex items-center gap-1.5 px-2 py-1 rounded-lg text-gray-500 hover:text-indigo-600 hover:bg-indigo-50 active:scale-95 transition">
                          {copiedMsgId === idx ? <Check size={14} className="text-emerald-500"/> : <Copy size={14} />}
                          <span className="text-[10px] font-bold">{copiedMsgId === idx ? 'Tersalin' : 'Salin'}</span>
                        </button>
                      </div>
                    )}
                  </div>
            ...
            {chatSending && <div className="w-full flex justify-start"><div className="max-w-[80%] rounded-2xl px-4 py-3 bg-white border border-gray-200 rounded-tl-sm shadow-sm"><TypingDots /></div></div>}
          </div>
